...
### Обновление production-сервера одной командой

После каждого обновления запускайте из папки `server`:

```powershell
npm run deploy
```

Команда выполняет `git pull --ff-only`, устанавливает зависимости сервера и клиента по lock-файлам, собирает frontend, применяет миграции и запускает сервер через `npm start`. При наличии локальных изменений или ошибке любого шага deployment останавливается.

Сервер работает в том же окне PowerShell. Перед следующим обновлением остановите его через `Ctrl+C`, снова выполните `npm run deploy` и не закрывайте окно после запуска.

Проверить последовательность команд без их выполнения можно через `npm run deploy:dry-run`.
...
- [ ] Интерактивная карта Plastkrep с расположением контейнеров
- [ ] Система уведомлений
