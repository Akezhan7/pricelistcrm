...
Один раз установите PM2 и включите его автозапуск (команду, которую выведет `pm2 startup`, также нужно выполнить):
...
Команда выполняет `git pull --ff-only`, устанавливает зависимости сервера и клиента по lock-файлам, собирает frontend, применяет миграции, запускает или перезапускает `plastkrep-crm` через PM2 и проверяет `/health`. При наличии локальных изменений или ошибке любого шага deployment останавливается.
