...
type CreateSupplierModalProps = {
  ...
  onClose: () => void;
  onSuccess: () => void;
  // optional selected product to auto-link newly created supplier to a product
...
export const CreateSupplierModal: React.FC<CreateSupplierModalProps> = ({
  ...
  if (formData.row) data.append('row', String(formData.row));
  if (formData.container) data.append('container', String(formData.container));
      
      if (image) {
        data.append('containerImage', image);
      }

      // If modal opened in context of a selected product, attach products array
      // so backend will create ProductSupplier link. Use sellingPrice if available, otherwise 0.
      if (selectedProduct) {
        const prod = {
          id: selectedProduct.id,
          price: (selectedProduct.sellingPrice as any) || (selectedProduct.costPrice as any) || 0,
          quantity: 0,
          isAvailable: true,
          notes: '',
        };
        data.append('products', JSON.stringify([prod]));
      }
    ...
    <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 p-4">
      <div className="bg-white rounded-lg w-full max-w-md max-h-[90vh] overflow-y-auto">
        <div className="flex items-center justify-between p-4 border-b border-gray-200">
          <h2 className="text-lg font-semibold text-gray-900">Добавить поставщика</h2>
          <button
            onClick={onClose}
        ...
        <form onSubmit={handleSubmit} className="p-4 space-y-4">
          ...
          {/* Перемещаем поля Ряд/Контейнер прямо после имени */}
          ...
          <div className="flex justify-end space-x-3 pt-4">
            ...
            >
              ...
              {loading ? 'Сохранение...' : 'Сохранить поставщика'}
            </button>
          </div>
        </form>
      </div>
